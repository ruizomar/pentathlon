...
@section('elemento')
    <div class="col-sm-8 col-sm-offset-2 contenedor">
        ...
        <div id="formimprimir" class="row">
            {{ Form::open(array('url' => 'condecoraciones/imprimir','role' => 'form','id' => 'imprimir','class' => 'hidden')) }}
                <div id="reconocimientos"></div>
                <div class="form-group col-sm-2 pull-right">
                {{ Form::button('Imprimir',array('class' => 'btn btn-success btn-sm','type' => 'submit','id' => 'bimprimir')) }}
                </div>
            {{ Form::close() }}
        </div>
        <div class="row">
...
@section('scripts2')
    ...
    function reconocimiento(reconocimientos){
        var html='';
        $.each(reconocimientos,function(index, val){
            html +='<label><input class="hidden" type="checkbox" name="'+val.id+'" checked></label>';
          });
        $('#reconocimientos').html(html);
